...
## 15. v0.3.0 — 动态授权层（Dynamic Authorization）

> **目标**：在静态 RBAC 之上增加运行时风险评估层，实现信任衰减、异常检测、领域约束和 DO-178C 启发的自主等级（L0–L4）。

### Phase 1 — 动态授权核心（P1a–P1j）

- [x] P1a: `Delegator` + `DelegatorType`（5 变体：Human/Agent/Service/System/Automated） — `src/rbac/dynamic/delegator.rs`
- [x] P1b: `ActionCategory`（7 级敏感度 0.1–1.0）+ `ActionRequest` — `src/rbac/dynamic/metrics.rs`
- [x] P1c: `AuthorizationVerdict` / `RiskScore` / `SubScores` / `AutonomyLevel` / `Strategy` / `ActionOutcome` — `src/rbac/dynamic/verdict.rs`
- [x] P1d: `TrustScore`（加权值、衰减、合规/违规反馈）+ `TrustScoreStore` trait + `InMemoryTrustScoreStore` — `src/rbac/dynamic/trust.rs`
- [x] P1e: `TaskDomain` + `DomainScope` + `DomainMatch`（in-domain/adjacent/out-of-domain）+ `trust_floor` — `src/rbac/dynamic/domain.rs`
- [x] P1f: `AnomalyDetector`（滑动窗口、z-score 偏差、Bernoulli 方差基线） — `src/rbac/dynamic/anomaly.rs`
- [x] P1g: `DynamicPolicy`（5 维权重 [0.10, 0.30, 0.25, 0.25, 0.10]、5 级自主阈值、per-level 策略） — `src/rbac/dynamic/policy.rs`
- [x] P1h: `AuthorizationArbiter`（authorize/risk_score/feedback/lockdown/restore）+ `frozen` HashSet — `src/rbac/dynamic/arbiter.rs`
- [x] P1i: `mod.rs` 导出 + prelude 更新
- [x] P1j: 141 单元测试全部通过

### Phase 2 — 审计重构与信任衰减

- [x] P2a: `AuditSink` trait + `InMemoryAuditSink`（异步追加、过滤查询、计数）
- [x] P2b: `AuditPolicyEngine` trait + `InMemoryAuditPolicyEngine`（规则匹配 + cooldown，支持 Denied/HighRisk/CategorySensitive/DomainMismatch/RapidDenials/TrustBelow/Composite 条件）
- [x] P2c: `AuditAnalyzer` trait + `DefaultAuditAnalyzer`（统计聚合）
- [x] P2d: `AuditLogger` 门面（组合三层 + 告警钩子）
- [x] P2e: `TrustDecayWorker`（`run_once()`/`run()`/`spawn()` + `list_ids()`）
- [ ] P2f: `PgTrustScoreStore`（需要 sea-orm + postgres 依赖）

### Phase 3 — AuthService 集成
...
- [ ] McpRouter 集成
- [ ] scepter Delegator 注入
- [ ] TUI 命令支持

### Phase 5 — IEPL TypeScript 策略编排

- [ ] TypeScript policy orchestration layer

### 关键设计决策

| 决策 | 原因 |
|------|------|
| `frozen: Arc<RwLock<HashSet<String>>>` 硬锁定集 | 纯风险评分无法可靠驱动至 L0（lockdown 设置 trust=0 → 加权=0 → trust_penalty=1.0 → risk≈0.33 = L3）。Frozen set 绕过风险计算，直接返回 Deny |
| `DomainScope::evaluate()` 简化为 `(&ActionCategory, Option<&str>)` | 降低耦合，避免依赖完整 `ActionRequest` |
| `AuditLogger` 是 struct 门面而非 trait | 用户按需组合 Sink + PolicyEngine + Analyzer + 告警钩子 |
| 新 Delegator 默认 trust=0.5, confidence=0.1 | 高 trust_penalty (0.95)，新主体从高风险开始，随证据积累降低 |
| 异常基线需要 100+ 样本 | 低于阈值时 anomaly 维度返回 0.0，权重减半（`anomaly_weight_modifier`）|
